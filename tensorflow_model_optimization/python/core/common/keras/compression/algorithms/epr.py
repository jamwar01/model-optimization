...
@tf.custom_gradient
def _to_complex_with_gradient(value):
  return tf.bitcast(value, tf.complex64), lambda g: tf.bitcast(g, tf.float32)


def _transform_dense_weight(weight, log_step, quantized=True):
...
def _transform_conv_weight(kernel_rdft, kernel_shape, log_step, quantized=True):
...
class EPR(algorithm.WeightCompressor):
  ...
  def _init_training_weights_reparam(self, pretrained_weight: tf.Tensor):
  ...
  def regularization_loss(self, *training_weights) -> tf.Tensor:
    if len(training_weights) == 3:
      # Bias or dense kernel.
      weight, log_step, log_scale = training_weights
    else:
...
class FastEPR(EPR):
  ...
  def __init__(self, regularization_weight, alpha=1e-2):
  ...
  def decompress_weights(self, string, weight_shape, log_step) -> tf.Tensor:
    weight_shape = tf.cast(weight_shape, tf.int32)
    log_step = tf.cast(log_step, tf.float32)
    if weight_shape.shape[0] <= 2:
      # Bias or dense kernel.
